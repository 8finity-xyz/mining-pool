...
// PoolRegistryRewardsFinalized represents a RewardsFinalized event raised by the PoolRegistry contract.
type PoolRegistryRewardsFinalized struct {
	Reward      *big.Int
	SubmitsCost *big.Int
	PoolFee     *big.Int
	Raw         *types.Log // Blockchain specific contextual infos
...
const PoolRegistryRewardsFinalizedEventName = "RewardsFinalized"

// ContractEventName returns the user-defined event name.
func (PoolRegistryRewardsFinalized) ContractEventName() string {
	return PoolRegistryRewardsFinalizedEventName
}

// UnpackRewardsFinalizedEvent is the Go binding that unpacks the event data emitted
// by contract.
//
// Solidity: event RewardsFinalized(uint256 reward, uint256 submitsCost, uint256 poolFee)
func (poolRegistry *PoolRegistry) UnpackRewardsFinalizedEvent(log *types.Log) (*PoolRegistryRewardsFinalized, error) {
	event := "RewardsFinalized"
	if log.Topics[0] != poolRegistry.abi.Events[event].ID {
		return nil, errors.New("event signature mismatch")
	}
	out := new(PoolRegistryRewardsFinalized)
	if len(log.Data) > 0 {
		if err := poolRegistry.abi.UnpackIntoInterface(out, event, log.Data); err != nil {
			return nil, err
		}
